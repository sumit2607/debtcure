...
OLD_LOADER = """
    ...
    <div id="page-loader">
        <div class="loader-content">
            <svg class="loader-icon pulse" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" width="60" height="60">
            ...
            <div class="shimmer-text">Loading DebtCure...</div>
        </div>
    </div>
"""
...
def replace_loader(directory):
    html_files = glob.glob(os.path.join(directory, '*.html'))
    for file_path in html_files:
        with open(file_path, 'r', encoding='utf-8') as f:
            content = f.read()

        if 'id="page-loader"' in content and '<style>' not in content.split('page-loader')[1][:1000]:
            content = content.replace(OLD_LOADER.strip(), NEW_LOADER.strip())
            with open(file_path, 'w', encoding='utf-8') as f:
                f.write(content)
            print(f"Fixed loader in {os.path.basename(file_path)}")

if __name__ == '__main__':
    replace_loader('/Users/sumit/Downloads/debtCure')
